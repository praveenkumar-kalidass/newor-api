...
describe('Client Service', () => {
  ...
  describe('authorize', () => {
    it('should successfully authorize client', async () => {
      ...
      await expect(clientService.authorize(mockContext, { id: 'test', secret: 'test@123' })).resolves.toStrictEqual(expectedResponse);
    });

    it('should throw client not found error', async () => {
      clietDao.fetchBy.mockResolvedValueOnce(null);

      await expect(clientService.authorize(mockContext, { id: 'test', secret: 'test' })).rejects.toStrictEqual({
        status: 404,
        data: {
    ...
    it('should throw error when authorize client', async () => {
      ...
      await expect(clientService.authorize(mockContext, { id: 'test', secret: 'test@123' })).rejects.toStrictEqual({
        status: 500,
        data: {
